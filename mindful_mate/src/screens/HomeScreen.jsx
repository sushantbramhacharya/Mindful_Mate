    ...
    <div className="min-h-screen w-screen bg-purple-100 text-gray-800 p-4">
      <nav className="flex justify-between items-center p-4 bg-purple-300 rounded-xl shadow-md mb-6">
        <div className="flex space-x-4">
          {['Home', 'Chat', 'Mood Tracker', 'Songs', 'Quotes'].map((item) => (
            <Button key={item} variant="ghost" className="rounded-full px-4 py-2 bg-purple-400 text-white hover:bg-purple-500">{item}</Button>
          ))}
        </div>
        <Button className="bg-purple-500 text-white px-4 py-2 rounded-full">Sign in</Button>
      </nav>
      ...
      <div className="bg-purple-200 p-6 rounded-3xl shadow-md">
        ...
        <div className="flex justify-between mb-6">
          {[...Array(5)].map((_, i) => (
            <div key={i} className="bg-purple-300 p-4 rounded-xl shadow hover:scale-105 transition-transform">
